...
def run_anthropic_review(packet: dict, settings: Settings) -> ProviderResponse:
    ...
    api_path = (
        settings.llm_api_path if settings.llm_api_path not in {"", "/responses"} else "/v1/messages"
    )
    ...
    try:
        ...
        AnthropicReviewResponse.model_validate(raw_response)
    ...
    content = extract_anthropic_text(raw_response)
